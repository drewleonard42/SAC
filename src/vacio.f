...
subroutine readparameters(w)
  ...
  namelist /testlist/   teststr,ixtest1,ixtest2,ixtest3,iwtest,idimtest,&
     ipetest     
  namelist /filelist/   filenameini,filename,varnames,      npe1,npe2,npe3,&
      typefileini,typefileout,typefilelog,snapshotini,snapshotout,fullgridini,&
     fullgridout,dixBmin1,dixBmin2,dixBmin3,dixBmax1,dixBmax2,dixBmax3
...
!!!
 ...
 !ixtest1,ixtest2,ixtest3 is given for the full grid if ipetest was not set explicitly
 ...
 !Erase test string for other processors unless ipetest >= npe (test all PEs)
  ...
  if (oktest) then 
           write(unitterm,*)'npe^D=',npe1,npe2,npe3
  ...
  if(globalixtest)call mpiix(ixtest1,ixtest2,ixtest3,ipetest)
...
subroutine readfileini_bin(w)
 ...
 ! PRINT*, ipe, filenameini, unitini
  ...
  do
     ...
     do iw=1,nwin
        read(unitini,iostat=ios)w(ixmin1:ixmax1,ixmin2:ixmax2,ixmin3:ixmax3,&
           iw)
     end do
     if(ios/=0)then
...
subroutine setixGixMix(ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3)
  ...
  include 'vacdef.f'
  integer:: ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3,qnx     
  !-----------------------------------------------------------------------------
  ...
  if(fullgridini)then
     ...
     if(ipe3==0)            nx(3)=nx(3)-dixBmin3
     if(ipe3==npe3-1)      nx(3)=nx(3)-dixBmax3
  ...
  if(fullgridini)then
     ...
     if(ipe1>0)ixmin1=ixMmin1
          ...
          if(ipe3>0)ixmin3=ixMmin3
     if(ipe1<npe1-1)ixmax1=ixMmax1
          if(ipe2<npe2-1)ixmax2=ixMmax2
          if(ipe3<npe3-1)ixmax3=ixMmax3
    
  else
     ixmin1=ixMmin1;ixmin2=ixMmin2;ixmin3=ixMmin3;ixmax1=ixMmax1
     ixmax2=ixMmax2;ixmax3=ixMmax3;
  endif
  ...
  if(ixGmax1>ixGhi1.or.ixGmax2>ixGhi2.or.ixGmax3>ixGhi3)then
     ...
     write(uniterr,*)'Stop: ixGhi=',ixGhi1,ixGhi2,ixGhi3
     write(uniterr,*)'Stop: ixGmax=',ixGmax1,ixGmax2,ixGmax3
  ...
  call MPI_allreduce(nx3,nxall3,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,ierrmpi)
  nxall3=nxall3*npe3/npe;
...
subroutine savefile(ifile,w)
  ...
  case(filelog_)
     ...
     case('default')
        call savefilelog_default(unitini+ifile,w,ixmin1,ixmin2,ixmin3,ixmax1,&
           ixmax2,ixmax3)
     CASE('special')
        CALL savefilelog_special(unitini+ifile,w,ixmin1,ixmin2,ixmin3,ixmax1,&
           ixmax2,ixmax3)
     CASE default
...
subroutine savefileout_asc(qunit,w,ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3)
  ...
  integer:: qunit,ixmin1,ixmin2,ixmin3,ixmax1,ixmax2,ixmax3,ix1,ix2,ix3,iw,&
     idim,ndimout
