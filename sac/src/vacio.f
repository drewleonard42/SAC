...
subroutine readparameters(w)
  ...
  namelist /filelist/   filenameini,filename,varnames, typefileini,&
     typefileout,typefilelog,snapshotini,snapshotout,fullgridini,fullgridout,&
     dixBmin1,dixBmin2,dixBmax1,dixBmax2
  ...
  namelist /methodlist/ wnames,fileheadout,eqpar,typeadvance,typefull,&
     typepred,typeimpl,typefilter,typelimiter,typeentropy,entropycoef,artcomp,&
...
!!!
  ...
  if(angmomfix)then
     write(*,*)'Angular momentum conservation is on (angmomfix=T) for iw=',&
        mphi_
...
subroutine readfileini_asc(w)
  ...
  do
     ...
     read(unitini,*,iostat=ios)(x(ix1,ix2,idim),idim=1,ndim),(w(ix1,ix2,iw),&
        iw=1,nwin),(wextra,iw=nwin+1,nwini)
     enddo;enddo;
     if(ios/=0)then
...
subroutine setixGixMix(ixmin1,ixmin2,ixmax1,ixmax2)
  ...
  if(ixGmax1>ixGhi1.or.ixGmax2>ixGhi2)then
     write(uniterr,*)'Stop: nxhi=',ixGhi1-dixBmax1-ixMmin1+1,ixGhi2-dixBmax2&
        -ixMmin2+1
     write(uniterr,*)'Stop: ixGhi=',ixGhi1,ixGhi2
     write(uniterr,*)'Stop: ixGmax=',ixGmax1,ixGmax2
...
subroutine savefile(ifile,w)
  ...
  case(fileout_)
     ...
     CASE('hdf5')
        call savefileout_hdf5(w,ixmin1,ixmin2,ixmax1,ixmax2)
     case default
  ...
  case(filelog_)
     ...
     case('default')
        call savefilelog_default(unitini+ifile,w,ixmin1,ixmin2,ixmax1,ixmax2)
     CASE('special')
        CALL savefilelog_special(unitini+ifile,w,ixmin1,ixmin2,ixmax1,ixmax2)
     CASE default
...
!**************** slice
integer:: s_ixmax1,s_ixmax2, prom_ixmax1,prom_ixmax2, s_ixmin1,s_ixmin2,&
    prom_ixmin1,prom_ixmin2  
!**************** endslice
...
SUBROUTINE open_hdf5()
  ...
  INCLUDE 'vacdef.f'
 ! IMPLICIT NONE
  !In VACDEF :: aspace_str, aspace_int, aspace_dbl, aspace_nx, aspace_eqpar
  !In VACDEF :: file_id, sac_group, time_group

  ! From get_aspace_hdf5:
  INTEGER(HID_T) :: astr_type
  ...
  info = MPI_INFO_NULL
  !IF not MPI the property access lists should still work.
  CALL h5pset_fapl_mpio_f(plist_id, MPI_COMM_WORLD, info, error)
  PRINT*, ipe, "created plist_id"
  PRINT*, ipe, filenameout
  ...
  CALL h5fcreate_f(TRIM(filenameout), H5F_ACC_TRUNC_F, file_id, error,&
      access_prp = plist_id) !filename out is in vacdef!?
  CALL h5pclose_f(plist_id, error)
    
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! Write inital file attributes !!!!!!!!!!!!!!!!!!!!!!!!!!!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! Get attribute dataspaces
  CALL get_aspace_hdf5(ndim, aspace_str, aspace_int, aspace_dbl, aspace_nx,&
      aspace_eqpar)
 ...
 !CALL h5acreate_f(file_id, "filehead", astr_type, aspace_str, attr_id, error, acpl_id = aplist_id)
  astr_data(1) = fileheadout
  !CALL h5awrite_f(attr_id, astr_type, astr_data, data_dims, error)
  !CALL h5aclose_f(attr_id, error)
  
!this is good but would need to be integrated into the par file somehow.
...
!!$  CALL h5aclose_f(attr_id, error)
 ...
 !RETURN -- Nothing -- Creates vars for VACDEF[[ plist_id, gplist_id, aplist_id, file_id, sac_group ]]
  PRINT*, "HDF5 Open"
 !ENDHDF
END SUBROUTINE open_hdf5

!=============================================================================
SUBROUTINE get_aspace_hdf5(ndim, aspace_str, aspace_int, aspace_dbl,&
    aspace_nx, aspace_eqpar)
       
  USE hdf5
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: ndim
  ! String attr varibles
  INTEGER(HID_T),INTENT(OUT) :: aspace_str
  INTEGER(HID_T) :: astr_type
  ...
  INTEGER     ::   aint_rank = 1
  INTEGER(HID_T),INTENT(OUT) ::  aspace_int
  !Set up attribute dataspace for string integer and float
  INTEGER(HSIZE_T), DIMENSION(1) :: adbl_dims = (/1/)
  INTEGER     ::   adbl_rank = 1
  INTEGER(HID_T),INTENT(OUT) :: aspace_dbl
  !Set up attribute dataspace for nx
  INTEGER(HSIZE_T), DIMENSION(1) :: anx_dims 
  INTEGER(HID_T),INTENT(OUT) ::  aspace_nx
  !Set up attribute dataspace for eqpar
  INTEGER(HSIZE_T), DIMENSION(1) :: aeq_dims = (/7/)
  INTEGER(HID_T),INTENT(OUT) :: aspace_eqpar
  INTEGER :: error
  anx_dims =  (/ndim/)
  ...
  !RETURN aspace_str, aspace_int, aspace_dbl, aspace_nx, aspace_eqpar,astr_type
END SUBROUTINE get_aspace_hdf5
}
!==============================================================================
SUBROUTINE savefileout_hdf5(w,ixmin1,ixmin2,ixmax1,ixmax2)
  INCLUDE 'vacdef.f'
  ...
  CHARACTER(LEN=80), DIMENSION(1) ::  astr_data
  INTEGER :: error, status, fierr
       
  INTEGER(HSIZE_T), DIMENSION(ndim + 1) ::  wdims, xdims, xcount, wcount,&
      hoffset
  INTEGER(HSIZE_T), DIMENSION(ndim + 1) :: np_xdims,  np_wdims
  INTEGER(HID_T) :: astr_type
  INTEGER(HID_T) :: attr_id 
  INTEGER(HSIZE_T), DIMENSION(1) :: data_dims
  ...
  INTEGER(HID_T) :: splist_id
  INTEGER(SIZE_T) :: astr_len = 80
       
  nx = (/ixmax1-ixmin1,ixmax2-ixmin2/)
  wdims = (/ixGmax1-ixGmin1,ixGmax2-ixGmin2,nw/)
  xdims = (/ixGmax1-ixGmin1,ixGmax2-ixGmin2,ndim/)
  xcount = (/ixmax1-ixmin1,ixmax2-ixmin2,ndim/)
  wcount = (/ixmax1-ixmin1,ixmax2-ixmin2,nw/)
  np_xdims = (/ixmax1-ixmin1,ixmax2-ixmin2,ndim/)
  np_wdims = (/ixmax1-ixmin1,ixmax2-ixmin2,nw/)

  PRINT*, "save file  out hdf5"
  PRINT*, "-----------------------"
  PRINT*, "ixmin", ixmin1,ixmin2
  PRINT*, "ixmax", ixmax1,ixmax2
  PRINT*, "ixGmin",ixGmin1,ixGmin2
  PRINT*, "ixGmax", ixGmax1,ixGmax2
  PRINT*, "ixMmin", ixMmin1,ixMmin2
  PRINT*, "ixMmax", ixMmax1,ixMmax2
  PRINT*, "full grid?", ixmin1,ixmin2,ixmax1,ixmax2
  PRINT*, "======================="
  PRINT*, "xcount", xcount
  PRINT*, "np_xdims", np_xdims
  PRINT*, "xdims", xdims
 

  !     
  !nx = (/ixmax1-ixmin1,ixmax1-ixmin1/)
  !wdims = (/ ixMmax1 * npe1,ixMmax1 * npe1,nw/)!this only existis in MPI mode
  !xdims = (/ ixMmax1 * npe1,ixMmax1 * npe1,ndim/)
  !xcount = (/ixmax1-ixmin1,ixmax1-ixmin1,ndim/)
  !wcount = (/ixmax1-ixmin1,ixmax1-ixmin1,nw/)
  !np_xdims = (/ixmax1-ixmin1,ixmax1-ixmin1,ndim/)
  !np_wdims = (/ixmax1-ixmin1,ixmax1-ixmin1,nw/)
  !
  !PRINT*, "save file  out hdf5"
  !PRINT*, "-----------------------"
  !PRINT*, ipe,npe, npe1
  !PRINT*, ipe,"ixmin", ixmin1
  !PRINT*, ipe,"ixmax", ixmax1
  !PRINT*, ipe,"ixGmin",ixGmin1
  !PRINT*, ipe,"ixGmax", ixGmax1
  !PRINT*, ipe,"ixMmin", ixMmin1
  !PRINT*, ipe,"ixMmax", ixMmax1
 !PRINT*, ipe,"full grid?", ixmin1 * npe1,ixmin1 * npe1,ixmax1 * npe1,ixmax1 * npe1
  !PRINT*, "======================="
  !PRINT*, ipe,"xcount", xcount
  !PRINT*, ipe,"np_xdims", np_xdims
  !PRINT*, ipe,"xdims", xdims
  !,     
  !nx = (/ixmax2-ixmin2,ixmax2-ixmin2/)
  !wdims = (/ ixMmax2 * npe2,ixMmax2 * npe2,nw/)!this only existis in MPI mode
  !xdims = (/ ixMmax2 * npe2,ixMmax2 * npe2,ndim/)
  !xcount = (/ixmax2-ixmin2,ixmax2-ixmin2,ndim/)
  !wcount = (/ixmax2-ixmin2,ixmax2-ixmin2,nw/)
  !np_xdims = (/ixmax2-ixmin2,ixmax2-ixmin2,ndim/)
  !np_wdims = (/ixmax2-ixmin2,ixmax2-ixmin2,nw/)
  !
  !PRINT*, "save file  out hdf5"
  !PRINT*, "-----------------------"
  !PRINT*, ipe,npe, npe2
  !PRINT*, ipe,"ixmin", ixmin2
  !PRINT*, ipe,"ixmax", ixmax2
  !PRINT*, ipe,"ixGmin",ixGmin2
  !PRINT*, ipe,"ixGmax", ixGmax2
  !PRINT*, ipe,"ixMmin", ixMmin2
  !PRINT*, ipe,"ixMmax", ixMmax2
 !PRINT*, ipe,"full grid?", ixmin2 * npe2,ixmin2 * npe2,ixmax2 * npe2,ixmax2 * npe2
  !PRINT*, "======================="
  !PRINT*, ipe,"xcount", xcount
  !PRINT*, ipe,"np_xdims", np_xdims
  !PRINT*, ipe,"xdims", xdims
  !
  ! Check file is valid
  !CALL H5Fis_hdf5(filenameout,status,error)
  !IF (NOT(status)) THEN !If file is not hdf5
  !   CALL die("Target HDF file is not HDF5")
 ! END IF
  IF (error < 0) THEN
     CALL die("Target HDF5 file can not be read [Probably not created]")
  END IF
  ...
  CALL ipe2ipeD(ipe,ipe1)
  hoffset(1) = ipe1 * nx(1)
       hoffset(1) = ipe1 * nx(1)
  hoffset(4) = 0      
  CALL ipe2ipeD(ipe,ipe2)
  hoffset(2) = ipe2 * nx(2)
       hoffset(2) = ipe2 * nx(2)
  hoffset(4) = 0
  PRINT*, hoffset
  PRINT*, "----------------------"
  !CALL die("")
  IF (first_file_write) THEN
     !Write attributes to main group
     !neqpar, eqpar, nt, its, ndim
     !neqpar
     CALL h5acreate_f(sac_group, "neqpar", H5T_NATIVE_INTEGER, aspace_int,&
         attr_id, fierr, acpl_id = aplist_id)
     CALL h5awrite_f(attr_id, H5T_NATIVE_INTEGER, neqpar, data_dims, fierr)
     CALL h5aclose_f(attr_id, error)
     !eqpar
     CALL h5acreate_f(sac_group, "eqpar", H5T_NATIVE_DOUBLE, aspace_eqpar,&
         attr_id, fierr, acpl_id = aplist_id)
     CALL h5awrite_f(attr_id, H5T_NATIVE_DOUBLE, eqpar, data_dims, fierr)
     CALL h5aclose_f(attr_id, error)
     !ndim
     CALL h5acreate_f(sac_group, "ndim", H5T_NATIVE_INTEGER, aspace_int,&
         attr_id, fierr, acpl_id = aplist_id)
     CALL h5awrite_f(attr_id, H5T_NATIVE_INTEGER, ndim, data_dims, fierr)
     CALL h5aclose_f(attr_id, error)
     !nx
     CALL h5acreate_f(sac_group, "nx", H5T_NATIVE_INTEGER, aspace_nx, attr_id,&
         fierr, acpl_id = aplist_id)
     ...
     CALL h5dcreate_f(sac_group, "x", H5T_NATIVE_DOUBLE, xf_space, dset_id,&
         error, plist_id)
     ...
     CALL h5sselect_hyperslab_f (xf_space, H5S_SELECT_SET_F, hoffset, xcount,&
         error)
     ...
     CALL h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, x, np_xdims, error, &
          file_space_id = xf_space, mem_space_id = xm_space, xfer_prp &
             = plist_id)
     ...
     CALL h5gcreate_f(sac_group, "wseries", time_group, error, gapl_id &
        = gplist_id)
     
     !Write varnames and nw to time series group
     CALL h5tcopy_f(H5T_NATIVE_CHARACTER, astr_type, error)
     CALL h5tset_size_f(astr_type, astr_len, error)
     CALL h5acreate_f(time_group, "varnames", astr_type, aspace_str, attr_id,&
         fierr, acpl_id = aplist_id)
     astr_data(1) = varnames
     CALL h5awrite_f(attr_id, astr_type, astr_data, data_dims, fierr)
     CALL h5aclose_f(attr_id, error)
     
     CALL h5acreate_f(time_group, "nw", H5T_NATIVE_INTEGER, aspace_int,&
         attr_id, fierr, acpl_id = aplist_id)
     CALL h5awrite_f(attr_id, H5T_NATIVE_INTEGER, nw, data_dims, fierr)
     CALL h5aclose_f(attr_id, error)
     
     !Set first wirite flag complete
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  WRITE (wname, "(A1,I0.5)") "w", it!?????!
  PRINT*, "wname", wname
  ! Create chunked dataset.
  CALL h5dcreate_f(time_group, wname, H5T_NATIVE_DOUBLE, wf_space, dset_id,&
      error, plist_id)
  CALL h5sclose_f(wf_space, error)
  
  ! Select hyperslab in the file.
  CALL h5dget_space_f(dset_id, wf_space, error)
  CALL h5sselect_hyperslab_f (wf_space, H5S_SELECT_SET_F, hoffset, wcount,&
      error)
     
  ! Write W dataset collectively. 
  CALL h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, w, np_wdims, error, &
       file_space_id = wf_space, mem_space_id = wm_space, xfer_prp &
          = splist_id)
  
  !Write it and t to dataset
  !t
  CALL h5acreate_f(dset_id, "t", H5T_NATIVE_DOUBLE, aspace_dbl, attr_id,&
      fierr, acpl_id = aplist_id)
  CALL h5awrite_f(attr_id, H5T_NATIVE_DOUBLE, t, data_dims, fierr)
  CALL h5aclose_f(attr_id, error)
  !iter
  CALL h5acreate_f(dset_id, "it", H5T_NATIVE_INTEGER, aspace_int, attr_id,&
      fierr, acpl_id = aplist_id)
  ...
  ! Close the dataset.
  CALL h5dclose_f(dset_id, error)
  ! Close w array dataspaces.
  CALL h5sclose_f(wf_space, error)
  CALL h5sclose_f(wm_space, error)
 !END IFHDF
END SUBROUTINE savefileout_hdf5
!==============================================================================
